...
### 18.30 SDP / GW 与最优解质量的正式关系

本节根据 `SDP_maxcutratio.md` 的问答记录，把 MaxCut-3 后续评估口径固定下来，避免把 cut fraction、GW baseline、SDP upper bound 和 strict approximation ratio 混用。
...
```text
W        = total edge weight，当前无权 3-regular 图中 W = |E| = 3n/2
C(x)    = 某个离散 bitstring 的 cut value
C_star  = 真实最优 MaxCut 值
SDP_UB  = Goemans-Williamson SDP 松弛得到的上界
C_GW    = GW/SDP rounding 后得到的离散 cut value
```
...
其中 `SDP_UB` 是连续向量松弛问题的最优值，不是 GW rounding 后的离散解。GW 算法包含两步：

```text
1. 解 SDP，得到向量解和 SDP_UB；
2. 随机超平面 rounding，得到离散解 C_GW。
```

因此不能把 `C_GW` 当成最优值，也不能把 `SDP_UB` 叫作 GW 算法最终割值。
...
ratio_to_upper_bound:
  C / UB
  其中 UB 可以是 SDP_UB、CP-SAT upper bound、MILP dual bound 等。
...
```text
C/W 是割比例，只说明总边里切掉了多少比例。
它直观、稳定、方便快速扫模型，但不是严格近似比。

C/C_star 才是严格 approximation ratio。
只有 exact solver 证明 OPTIMAL，或者已知数学最优值时，才能这么写。

C/UB 是真实近似比的保守下界。
因为 C_star <= UB，所以 C/UB <= C/C_star。

C/C_best_known 只是相对当前最好已知离散解的分数。
如果 C_best_known 还不是 C_star，它不能作为严格近似比。
```

报告里禁止只写 `ratio`。必须写清楚分母：
...
#### 18.30.3 为什么文献里 GW 可能出现 0.95

Goemans-Williamson 的经典保证约为：

```text
E[C_GW] / C_star >= 0.87856
```

这表示最坏情况下的理论保证，不表示每个随机 3-regular 图上 GW 的实际表现只能到 0.878。

在随机 3-regular MaxCut 上，经验现象通常是：

```text
最优 cut fraction C_star / W 约为 0.935 到 0.937；
GW rounding + local search 的 cut fraction 常在 0.90+；
GW 离散解相对 SDP_UB 或 C_star 的比值常可到 0.94 到 0.96。
```

因此文献或实验中看到 `GW approximation ratio ~= 0.95` 并不矛盾。它通常表示：

```text
C_GW / C_star
```

或者在大图中用 SDP 上界替代未知最优值时的保守估计：

```text
C_GW / SDP_UB
```

它不是说 GW 的理论最坏情况保证从 0.878 变成了 0.95。

#### 18.30.4 当前 classical 文件夹的角色
...
```text
1. 生成与 SQNN 相同的 random 3-regular MaxCut 实例；
2. 用 CP-SAT 求 exact optimum 或 incumbent / upper bound；
3. 用 low-rank GW-style vector relaxation 计算 expected hyperplane cut：
   `sum_edges arccos(v_i dot v_j) / pi`，这是对齐论文 Figure 4 灰线的主 baseline；
4. 额外保存 sampled-best hyperplane cut 和 sampled-best + 1-bit greedy，
   作为更强但不完全对齐论文口径的启发式参考。
```

注意：

```text
当前代码中的 GW-style 不是 certified SDP-GW。
它是低秩 Burer-Monteiro 风格的向量松弛 + rounding 工程对照。
没有完整 SDP solver 证书时，必须写成 GW-style baseline，不能写成 certified GW。

论文 Figure 4 的 GW 灰线是 expected GW cuts，
即对所有随机超平面 rounding 的期望值。
因此本项目主 baseline 应写成 `GW-style expected hyperplane cut`。
如果报告 sampled-best hyperplane 或 1-bit greedy 后处理，
必须单独写成 `GW-style sampled-best` 或 `GW-style + 1-bit greedy`，
不能把它们混写成论文口径的 GW baseline。
```

后续如果接入 SDPNAL+、MOSEK、CSDP 或其他 SDP 求解器，需要额外保存：
...
这样才能把 GW 的两部分：`SDP upper bound` 和 `rounding discrete baseline` 分开报告。

#### 18.30.5 当前 256 / 512 / 1024 结果的正确解读

当前输出：

```text
outputs/classical_maxcut3/overall_report.md
```

同一套 `n, degree=3, seed=42` 的结果：

| n | W | exact / bound 状态 | 当前最优信息 | GW expected C/W | GW sampled-best C/W | GW+greedy C/W | SQNN+greedy C/W | 可报告的近似比 |
|---:|---:|---|---|---:|---:|---:|---:|---|
| 256 | 384 | CP-SAT OPTIMAL | C_star = 351 | 0.878184 | 0.911458 | 0.914062 | 0.901042 | GW expected = 0.960748, SQNN = 0.985755 |
| 512 | 768 | CP-SAT FEASIBLE | 711 <= C_star <= 719 | 0.888859 | 0.919271 | 0.923177 | 0.886719 | 只能报告 C/UB：GW expected >= 0.949435, SQNN >= 0.947149 |
| 1024 | 1536 | CP-SAT FEASIBLE | 1407 <= C_star <= 1455 | 0.885598 | 0.902344 | 0.916016 | 0.879557 | 只能报告 C/UB：GW expected >= 0.934899, SQNN >= 0.928522 |

解释：

```text
n=256:
  CP-SAT 已证明 OPTIMAL，所以 C/C_star 是严格近似比。
  论文口径 GW expected 的严格近似比为 0.960748。

n=512:
  CP-SAT 找到 cut=711，上界为 719。
  因此真实最优值在 [711, 719] 之间。
  不能把 711 直接写成 C_star。
  论文口径 GW expected 相对 CP-SAT 上界的保守比值为 0.949435。

n=1024:
  CP-SAT incumbent 为 1403，但 GW-style 找到 1407。
  因此 best-known 下界应更新为 1407，而不是 1403。
  当前只能写 1407 <= C_star <= 1455。
  论文口径 GW expected 相对 CP-SAT 上界的保守比值为 0.934899。
```

#### 18.30.6 后续计划更新

后续 MaxCut-3 主线实验统一采用双层报告：
...
正式对标：
  C/C_star          如果 exact optimum 可得
  C/SDP_UB          如果有 certified SDP 上界
  C/CP_SAT_UB       如果只有 CP-SAT / MILP upper bound
  C/C_best_known    只作为相对当前最好离散解，不叫 strict approximation ratio
```
...
C_best_known
exact_status
exact_or_cp_sat_incumbent
upper_bound
SQNN C/W
GW-style C/W
SQNN C/UB
GW-style C/UB
SQNN C/Cstar       only if exact_status == OPTIMAL
GW-style C/Cstar   only if exact_status == OPTIMAL
```

下一步优先级：

```text
1. 保留当前 CP-SAT exact / upper-bound 流程，用于 256/512/1024 实例的可复查记录。
2. 为 random 3-regular MaxCut 接入真正 SDP upper bound 求解器，得到实例专属 SDP_UB。
3. 把 `classical/maxcut3_compare.py` 的输出表扩展为：
   C/W、C/Cstar、C/CP_SAT_UB、C/SDP_UB、C/C_best_known 同时保存。
   GW-style expected、GW-style sampled-best 和 GW-style + 1-bit greedy 必须分列保存。
4. 后续所有图表中，GW-style 是离散 classical baseline；
   SDP_UB 或 CP-SAT upper bound 是分母/证书，不是 baseline 解本身。
```

### 18.31 SQNN 三类主指标：direct / direct-greedy / sample

从本节开始，MaxCut-3 主线中 SQNN 的结果必须按三种读出口径分开记录。这里按用户约定：

```text
C 表示 cut fraction，也就是割比 C_value / W；
C* 表示同一个图的最优割比 C_star / W；
R 表示近似比 R = C / C*。
```
...
```text
R = (C_value / W) / (C_star / W)
  = C_value / C_star
```

也就是说，用割比算近似比和用原始割值算近似比是等价的，但报告时必须写清楚当前的 `C` 是割比。

#### 18.31.1 SQNN direct：直接二值读出
...
```text
这是最干净的 SQNN deterministic readout。
不采样，不接 greedy，不做局部搜索。
当前主模型质量优先看 C_d 和 R_d。
```

#### 18.31.2 SQNN direct-greedy：直接二值读出 + greedy
...
```text
这是 SQNN direct 后接经典 1-bit greedy 的后处理结果。
它反映“SQNN 给出的初始点 + 局部修复”的工程效果。
它不能单独代表 SQNN 概率态本身。
```

#### 18.31.3 SQNN sample：采样取最优
...
```text
这是从 SQNN 概率分布中采样 K 次，然后取最优 bitstring。
默认 C_s 不接 greedy；如果以后要接 greedy，必须另写成 C_sg / R_sg。
报告 C_s 时必须同时记录 K，例如 sample_K=256、8192。
```

#### 18.31.4 与 GW baseline 的对比规则

论文 Figure 4 的 GW 灰线是：
...
```text
SQNN direct:
  C_d / R_d  对比  GW expected baseline

SQNN directgreedy:
  C_dg / R_dg  也对比  GW expected baseline
  但必须注明 directgreedy 含有 1-bit greedy 后处理。

SQNN sample:
  C_s / R_s  对比  GW sampled-best baseline
  二者都属于 best-of-K sampling/readout 口径。
```
...
但它不是论文 Figure 4 的灰线口径，只能作为更强 classical heuristic baseline。

#### 18.31.5 为什么 GW 也可以有 sampling

GW 的半正定规划 / 向量松弛部分本身不是采样，它输出的是一组单位向量：
...
```text
随机选一个 hyperplane；
根据 v_i 落在 hyperplane 哪一侧决定 x_i = 0/1。
```
...
```text
GW expected:
  不实际采样 hyperplane，直接用 arccos(v_i dot v_j)/pi 计算期望割值。
  这是论文 Figure 4 的主 baseline。

GW sampled:
  实际采 K 个随机 hyperplanes，得到 K 个 cut，然后可报告平均值或最好值。
  如果和 SQNN sample 对比，应使用同样的 K，并明确写成 sampled-best。

GW + greedy:
  从某个 sampled GW cut 出发再做 1-bit greedy。
  这是额外后处理，不是标准 GW expected baseline。
```
...
并必须同时写明 sample count `K`。

#### 18.31.7 当前三组规模的重新评估结果

当前目录中可复查的规模结果为：

```text
outputs/classical_maxcut3/n256_d3_s42
outputs/classical_maxcut3/n512_d3_s42
outputs/classical_maxcut3/n1024_d3_s42
```

本次按 `C_d, C_dg, C_s` 重新评估，`C_s` 使用：
...
| n | exact / bound 状态 | GW expected C | GW sampled-best C | C_d | C_dg | C_s | R_d | R_dg | R_s |
|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 256 | CP-SAT OPTIMAL, C_star=351 | 0.878184 | 0.911458 | 0.893229 | 0.901042 | 0.893229 | 0.977208 | 0.985755 | 0.977208 |
| 512 | CP-SAT FEASIBLE, 711 <= C_star <= 719 | 0.888859 | 0.919271 | 0.871094 | 0.886719 | 0.876302 | - | - | - |
| 1024 | CP-SAT FEASIBLE, 1407 <= C_star <= 1455 | 0.885598 | 0.902344 | 0.873698 | 0.879557 | 0.871094 | - | - | - |

说明：

```text
1. n=256 已证明 exact optimum，所以 R_d/R_dg/R_s 是严格近似比。
2. n=512 和 n=1024 仍只有上下界，不能报告严格 R；
   当前图中 ratio 图使用 C/CP-SAT_UB 作为保守下界。
3. C_d 是当前 SQNN 主指标；
   C_dg 是带 greedy 的工程后处理；
   C_s 是 K=256 的采样取最优，不接 greedy。
```

本次输出图：

```text
sqnn_cut_fraction_by_round.png
sqnn_ratio_by_round.png
sqnn_energy_by_round.png
sqnn_vs_gw_round_trace.png
```

### 18.32 512 变量图的 C* 分母策略更新
...
```text
CP-SAT incumbent = 711
CP-SAT upper bound = 719
因此 711 <= C* <= 719
```
...
```text
1. MaxSAT / RC2:
   将每条边 (i,j) 写成两个 soft clauses:
     (x_i or x_j)
     (not x_i or not x_j)
   若边被割开，两条 soft clauses 都满足；
   若没有割开，只满足一条。
   因此 MaxSAT 最优值可还原 MaxCut。
   但该路线在 256 节点校验上短时间内也未完成，不适合作为当前主路线。

2. OR-Tools SCIP MIP:
   256 节点可证明 C*=351；
   512 节点 300 秒只得到 incumbent=706, upper_bound≈734.48，
   比 CP-SAT 当前 711/719 更弱。

3. CP-SAT decision target:
   直接询问是否存在 cut >= 712。
   300 秒返回 UNKNOWN，未能证明 C*=711。
```

所以当前不能把 `711` 写成严格 `C*`。对于论文式比较，应优先报告保守分母：
...
```text
C_GW_expected = 682.6434936523438
R_GW_lower = 682.6434936523438 / 719 = 0.9494346
```

若用当前 best-known feasible cut 作为分母，则得到：

```text
R_GW_to_best_known = 682.6434936523438 / 711 = 0.9601174
```

因此真实的 `R_GW = C_GW_expected / C*` 位于：

```text
0.9494346 <= R_GW <= 0.9601174
```

这已经覆盖 Google 文章中随机 3-regular 图上 GW 约 `0.95~0.96` 的量级。

理论/典型随机 3-regular 最优割比例可作为参考分母：

```text
C*_typical ≈ (0.935~0.937) * |E|
           ≈ 718.08 ~ 719.616
```

它与 CP-SAT 的实例级上界 `719` 一致，但它不是当前具体图的严格证明值。
后续报告时命名规则：

```text
C/C*      只在 exact OPTIMAL 时使用。
C/UB      当只有实例级上界时使用，是保守近似比下界。
C/C_best  只表示相对 best-known incumbent，不能称为严格近似比。
C/C_typ   只表示相对随机 3-regular 理论/典型分母。
```

### 18.33 512 变量十个随机图：只使用 GW expected baseline

本轮按用户要求去掉 `GW sampled-best` 和 `GW+greedy` 作为经典 baseline，
只保留 Google 文章口径的：
...
四个 SQNN 指标均按 `C/W` 报告，不在这里声称 `C/C*`：

```text
SQNN expected C[p]  不二值读出，直接用概率 p_i 计算期望割值；
                    等价于 -E[p]，其中 E[p] 是独立 Bernoulli 概率下的 QUBO expected energy。
SQNN C_d            直接二值读出。
SQNN C_dg           直接二值读出 + 1-bit greedy。
SQNN C_s            SQNN Bernoulli sample best-of-K，本轮 K=256。
```

十个图的均值：

```text
GW expected       0.880544
SQNN expected    0.869120
SQNN C_d         0.877083
SQNN C_dg        0.884766
SQNN C_s         0.879297
```
...
```text
n512_10seeds_best_metrics_vs_gw_expected.png
n512_10seeds_gap_to_gw_expected.png
n512_10seeds_round_traces_vs_gw_expected.png
seed_k/sqnn_four_metrics_vs_gw_expected.png
seed_k/sqnn_energy_vs_gw_expected.png
```

### 18.34 性能提升方向：边耦合增强与多 symmetry seed ensemble
...
其中 `z_edge_cavity` 的含义是：

```text
对每条有向边 i -> j 保存一个 Z-message；
如果 i 当前更相信自己取 +1，那么 i -> j 会建议 j 更偏向 -1；
更新 message 时排除 j -> i 的直接回流，形成 non-backtracking / cavity 效果。
```

这已经是在增强“相邻节点反相关”的表达。下一步更强的边耦合路线不是只调 gain，
而是把边消息从一个标量升级成可学习 edge hidden state：
...
```text
1. 对 j 的 anti-correlation 建议；
2. 对本轮 RY/collapse 强度的门控。
```

预期作用：

```text
1. 让模型记住每条边在多轮传播中的状态，而不是只用当前 p_i/p_j；
2. 减少局部贪心才能修好的单点错误；
3. 让 C_d 更接近当前 C_dg，从而证明模型本体变强。
```

#### 18.34.2 已接入：multi-head / 多 symmetry seed ensemble

本轮已把已有的 `MultiHeadPhaseAwareSQNN` 接入 classical 评估链路：
...
```text
1. 每个 head 是同一个 SQNN 结构；
2. 每个 head 使用不同 symmetry_seed；
3. 多个 head 独立跑完整 SQNN dynamics；
4. 最终在 logit(p) 空间学习加权混合；
5. 输出仍然按 SQNN expected / C_d / C_dg / C_s 四个指标评估。
```

为什么优先做这个：

```text
当前 10 个 n=512 图中只有少数图明显优于 GW expected，
说明性能不稳定可能来自破对称/初始化路径。
multi-head 不改经典 baseline，也不引入 GW teacher，
只是让 SQNN 自己从多个 symmetry seed 中形成更稳定的概率读出。
```

已做 smoke test：

```text
outputs/smoke_n512_head2
```

该 smoke 只验证训练、保存、加载、出图链路，不用于性能结论。

### 18.35 RY / RZ 旋转角度的计算与物理含义

当前 V14 主模型把每个 MaxCut 变量看成一个 Bloch 向量：
...
真正决定最终 bit 倾向的是 Z；
RZ 主要改变 X/Y 平面的相位；
RY 会把 X/Z 互相旋转，所以 RY 才能直接改变 Z 和 p_i。
```

#### 18.35.1 一轮更新的顺序

第 `t` 轮不是直接给每个节点一个独立概率，而是先为每个节点计算两个旋转角：
...
然后按下面顺序作用在 Bloch 向量上：

```text
1. 先做 RZ：在 X/Y 平面旋转，Z 不直接变。
2. 再做 RY：在 X/Z 平面旋转，Z 会改变。
3. 从新的 Z 得到新的概率 p_i。
4. 用新的概率计算 expected MaxCut energy。
5. 如果 monotone_accept 开启，只有能量不变差时才接受整轮更新。
```
...
RZ 的基础信号来自当前局部场，或者局部场的时间记忆。

局部场可理解为：

```text
如果只看当前概率 p，改变 p_i 对 expected QUBO energy 的局部影响方向。
```

模型当前使用 memory 模式，所以先形成一个相位记忆：

```text
phase_memory_i(t)
  = phase_memory_decay * phase_memory_i(t-1)
    + local_field_i(t)
```
...
```text
phi_i(t)
  = phase_steps(t) * phase_memory_i(t)
```
...
是第 `t` 轮所有节点共享的可训练参数，不是每个节点单独一个参数。

当前主线还打开了 XY feedback，因此会额外加入当前 Bloch 向量在 X/Y 平面里的相位：

```text
xy_phase_i(t) = atan2(Y_i, X_i)
```
...
它不直接改变 p_i；
它改变 X/Y 相位；
后续 RY 会把这部分相位信息折回 Z，从而间接影响 p_i。
```
...
```text
1. mixer_bias(t)：第 t 轮所有节点共享的偏置；
2. local_field_i(t)：概率能量给出的局部场方向；
3. relation_signal_i(t)：边耦合 / 相邻节点反相关信号。
```
...
theta_i(t)
  = mixer_bias(t)
    - field_steps(t) * local_field_i(t)
    + collapse_steps(t) * relation_signal_i(t)
```

其中：

```text
field_steps(t), mixer_bias(t), collapse_steps(t)
```

都是第 `t` 轮共享参数。不同节点的角度之所以不同，是因为：

```text
local_field_i(t) 不同；
relation_signal_i(t) 不同；
图结构和当前概率状态不同。
```

所以当前模型不是每个节点单独学习一个 RY 角度，而是学习：
...
其中 `z_edge_cavity` 会让 `relation_signal` 来自边消息聚合后的：
...
```text
邻居们通过边消息建议节点 i 应该往哪个二值方向走，
减去节点 i 当前自己的二值倾向。
```

如果邻居建议和当前倾向差距大，`z_edge_error_i` 就大；
如果邻居建议和当前倾向一致，`z_edge_error_i` 就小。

这个量不会直接进入 loss，也不会直接变成 bit。
它进入 RY 角度：
...
```text
边消息
  -> 聚合成节点 relation_signal
  -> 修正 RY 角度
  -> 改变 Bloch Z
  -> 改变概率 p_i
  -> 改变 expected energy / direct readout / sample readout
```

#### 18.35.5 collapse 为什么后期才打开
...
因此 collapse 大约从第 169 轮开始真正影响 RY。

设计意图：

```text
前期：主要依赖 local_field 和 RZ memory，让概率场先形成全局结构；
后期：把 z_edge_cavity 的边反相关建议折回 Z，让概率更二值、更贴近 MaxCut。
```
...
```text
第 1~168 轮：
  模型仍会计算并更新 z_edge_cavity 的边消息记忆，
  但这些边消息不会通过 collapse 项加进 RY 角度。

第 169~280 轮：
  relation_signal = z_edge_error
  会通过 collapse_steps(t) * relation_signal_i(t)
  加到每个节点的 RY 角度里，
  从而真正改变 Z 和概率 p_i。
```
...
```text
collapse_steps 前期虽然有数值，但不真正进入 RY；
黑色虚线之后，collapse_steps 才开始影响概率更新。
```

#### 18.35.6 从 RY / RZ 两个角度出发的优化方向
...
```text
让 relation_signal 更稳定、更有效地修正 RY，
从而减少 direct readout 和 directgreedy 之间的差距。
```

可尝试：

```text
1. 提高 z_message_gain，使边消息更果断；
2. 提高 collapse_init，使后期 relation_signal 对 RY 的影响更强；
3. 给 z_message_gain 做后期增大 schedule；
4. 引入节点级 RY gate，让不同置信度节点使用不同有效步长。
```

判断标准：

```text
如果 C_d 上升，并且 C_d 更接近 C_dg，
说明模型本体的读出更强，而不是只靠 greedy 修正。
```
...
```text
避免 RZ / XY feedback 后期过强或方向不稳定，
让 RZ 提供有用相位记忆，而不是把概率更新推向震荡。
```

可尝试：

```text
1. 降低 xy_feedback_init；
2. 降低 phase_memory_decay，让旧局部场记忆少一点；
3. 关闭 xy_feedback，只保留 memory + z_edge_cavity；
4. 测试更温和的 phase_steps 初始值。
```

判断标准：

```text
如果 SQNN expected 和 C_d 同时更平滑、更高，
说明 RZ 相位通道在帮助形成稳定概率场。
```

### 18.36 RY / RZ 角度优化小实验：n=512, seed=42

为了先筛方向，本轮只在当前固定图上做小实验：
...
```text
outputs/angle_optimization_n512_s42
```

脚本：

```text
classical/try_angle_optimizations.py
```
...
```text
1. ry_edge_boost:
   增强 z-edge/collapse 对 RY 的修正。

2. ry_learned_node_gate:
   让 RY 的 local-field 步长按节点状态动态缩放。

3. rz_soft_feedback:
   降低 phase_memory_decay 和 xy_feedback_init，
   让 RZ 相位反馈更温和。

4. rz_no_xy_feedback:
   关闭 XY feedback，只保留 memory + z_edge_cavity + collapse。
```

结果：

| variant | SQNN expected | C_d | C_dg | C_s | 结论 |
|---|---:|---:|---:|---:|---|
| baseline_current | 0.865696 | 0.871094 | 0.886719 | 0.876302 | 当前基线 |
| ry_edge_boost | 0.873706 | 0.885417 | 0.886719 | 0.884115 | 改善 expected/direct/sample，但仍略低于 GW expected |
| ry_learned_node_gate | 0.860444 | 0.869792 | 0.886719 | 0.871094 | 当前设置下无收益 |
| rz_soft_feedback | 0.863636 | 0.869792 | 0.872396 | 0.872396 | 比 baseline 更差 |
| rz_no_xy_feedback | 0.859956 | 0.893229 | 0.908854 | 0.885417 | direct/directgreedy 最好，C_d 超过 GW expected |

本图的 GW expected：

```text
GW expected C/W = 0.888859
```

阶段性判断：

```text
1. 最明显的收益来自 RZ 方向：关闭 XY feedback。
2. 这说明当前 XY phase feedback 可能在后期引入相位干扰；
   它不一定降低 expected energy，但会影响 direct readout 的二值质量。
3. ry_edge_boost 也有正向信号，尤其提高 SQNN expected / C_d / C_s；
   但在 seed=42 上还没有让 C_d 超过 GW expected。
4. learned_node_gate 当前不值得优先推进。
```

下一步：

```text
优先把 rz_no_xy_feedback 扩展到 n=512 的 10 个随机图。
若 10-seed 平均 C_d / C_dg 继续优于 GW expected，
则把它作为新的 V14-512 主分支；
否则说明该收益可能是 seed=42 特例。
```

模型简化原则：

```text
如果 10 个随机图上都显示 XY feedback 无收益或负收益，
后续主模型直接移除 XY feedback。
理由是：在不稳定收益的情况下，优先保留更简单、更可解释的结构；
让模型主要依赖 local-field memory + z_edge_cavity + late collapse。
```
### 18.37 n=512 / 10 seed mechanism scan: XY, phase memory, z-edge/collapse

This scan checks whether auxiliary mechanisms such as `xy_feedback`, `phase_memory`, and `collapse/z_edge_cavity` actually help on n=512 random 3-regular MaxCut. The baseline is GW expected hyperplane rounding, and all numbers below are cut fraction gaps to GW expected.
...
rounds = 280
epochs = 110
metrics = SQNN expected, C_d, C_dg, C_s
```
...
Current best complete 10-seed candidate:
...
z_message_gain         = 1.8
z_message_gain_final   = 2.6
z gain schedule start  = 0.55
rollback_aux_on_reject = False
```

Ten-seed result relative to GW expected:

| variant | direct gap | direct wins | sample gap | sample wins | expected gap | C_dg gap |
|---|---:|---:|---:|---:|---:|---:|
| `edge_boost_mem060_no_xy` | +0.009560 | 9/10 | +0.007216 | 8/10 | -0.008419 | +0.014508 |
| `memory_decay_0p60_no_xy` | +0.002008 | 6/10 | -0.000857 | 5/10 | -0.019547 | +0.014638 |
| `baseline_current` | -0.003461 | 1/10 | -0.001247 | 5/10 | -0.011424 | +0.004221 |

Mechanism decisions:

```text
1. Do not keep full-time XY feedback in the recommended clean route.
   The best route removes XY feedback entirely.

2. Do not simply delete phase memory.
   no_memory_no_xy is much worse.

3. Do not use long phase memory.
   phase_memory_decay=0.95 is unstable and weak.

4. phase_memory_decay=0.60 is the useful short-memory point.

5. z_edge/collapse should be strengthened, but only together with short memory.
   Strong edge boost with memory_decay=0.80 was unstable;
   strong edge boost with memory_decay=0.60 is the current best.

6. Early-XY then late-off was tested once and did not help.

7. rollback_aux_on_reject did not help in the half-run test and was stopped.
```

Interpretation:

```text
The improvement is not coming from a more complicated phase feedback system.
It comes from a cleaner MaxCut-specific path:

local-field short memory
+ directed z-edge anti-correlation message
+ stronger late collapse into RY

The expected probability cut is still below GW expected, so the probability
distribution itself is not yet better than GW. The gain is in readout quality:
C_d and C_s produce better bitstrings more often.
```

Code status:

```text
classical/scan_n512_mechanisms.py
    Added the 10-seed mechanism scan.

scripts/run_maxcut3_phase_aware_probe.py
    Added xy_feedback_active_fraction / xy_feedback_decay_fraction.
    Added rollback_aux_on_reject.

classical/maxcut3_compare.py
    Added recommended_clean_edgeboost_config.

classical/n512_10_random_graphs.py
    Default model_config is now clean_edgeboost_mem060.
    Old V14 route is still available with:
...
```text
1. Tune collapse_init and z_message_gain around clean_edgeboost_mem060.
2. Try multi-head / multi symmetry seed ensemble on clean_edgeboost_mem060.
3. Replace scalar z_edge message with a small shared edge hidden state.
